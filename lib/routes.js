const User = require('mongo/user');

exports.apply = function(app) {
  app.get('/', addRenderLocals, function(req, res) {
    res.render('index', {
      locals: {
        authDetails: req.getAuthDetails(),
        title: 'QandA'
      }
    });
  });

  app.get('/protected', ensureUser, addRenderLocals, function(req, res) {
    res.render('protected', {
      locals: {
        title: 'Protected page'
      }
    });
  });

  app.get('/register', addRenderLocals, function(req, res) {
    res.render('register', {
      locals: {
        title: 'Register',
        actionUrl: '/register'
      }
    });
  });

  app.post('/register', function(req, res) {
    var user = new User();
    user.user = req.body.user;
    user.name = req.body.name;
    user.salt = Math.floor(Math.random() * 0x100000000).toString(16);
    user.password = req.body.password;

    user.save(function() {
      res.redirect('/registered/' + user.user, 303);
    });
  });

  app.get('/registered/:user', addRenderLocals, function(req, res) {
    res.render('registered', {
      locals: {
        authDetails: req.getAuthDetails(),
        title: 'Registered',
        user: req.params.user
      }
    });
  });

  app.get('/logout', function(req, res) {
    req.logout();
    res.redirect('/', 303);
  });

  app.get('/checkIfUserUsed/:user', function(req, res){
    User.findByUser(req.params.user, function(user) {
      res.send(user ? 'Already in use' : '', { 'Content-Type': 'text/plain' });
    });
  });
}

function ensureUser(req, res, next) {
  req.authenticate(['user'], function(error, authenticated) {
    next();
  });
}

function addRenderLocals(req, res, next) {
  var originalRender = res.render;
  
  res.render = function() {
    var options = arguments[1];
    options.locals.authDetails = req.getAuthDetails();

    originalRender.apply(res, arguments);
  };
  
  next();
}
